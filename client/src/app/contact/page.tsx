...
export default function ContactPage() {
  ...
  /** mailto: ile mail istemcisini aç — her cihazda çalışır, backend gerektirmez */
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();

    const body = `Merhaba Fatih,\n\n${message}\n\n---\nGönderen: ${name}\nE-posta: ${email}`;
    const mailtoUrl =
  ...
  const inp =
    "w-full rounded-2xl border py-3 px-4 text-sm outline-none transition focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/20 placeholder:text-slate-500";
  const inpStyle = {
    ...
    <div className="mx-auto max-w-4xl space-y-12">
      <Reveal>
        <header className="space-y-3">
          ...
          <p className="max-w-lg text-sm leading-7" style={{ color: "rgb(var(--t3))" }}>
            Akademik işbirliği, proje fikri veya sadece sohbet etmek için
            formu doldur — mail istemcin açılacak ve mesajın <strong style={{ color: "rgb(var(--t2))" }}>{CONTACT_EMAIL}</strong> adresime gidecek.
          </p>
        </header>
      </Reveal>
      ...
      <div className="grid gap-8 lg:grid-cols-5">
            ...
            <div className="rounded-3xl border p-7" style={{ background: "rgb(var(--surface))", borderColor: "rgb(var(--surface2))" }}>
              ...
              {sent ? (
                <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
                  ...
                  <div className="text-sm font-bold text-emerald-300">Mail istemcin açıldı!</div>
                  <p className="mt-2 text-xs text-slate-400">
                    Mesajın hazırlandı. Mail uygulamanızdan göndermeyi onaylayın.
                  </p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <button
                    type="submit"
                    className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-indigo-500 to-violet-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition hover:brightness-110"
                  >
                    <Mail className="h-4 w-4" />
                    Mail İstemcini Aç & Gönder
                  </button>
                  <p className="text-center text-xs" style={{ color: "rgb(var(--t3))" }}>
                    Butona basınca mail uygulamanız açılır ve mesaj {CONTACT_EMAIL} adresine hazır gelir.
                  </p>
                </form>
              )}
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <Reveal delay={0.08}>
            <div className="rounded-3xl border p-6" style={{ background: "rgb(var(--surface))", borderColor: "rgb(var(--surface2))" }}>
              ...
              <Link
                ...
                className="block text-sm font-semibold text-indigo-400 hover:text-indigo-300 transition underline underline-offset-2"
              >
                {CONTACT_EMAIL}
              </Link>
              <p className="mt-2 text-xs leading-5" style={{ color: "rgb(var(--t3))" }}>
                Akademik işbirliği, proje soruları veya iş teklifleri için doğrudan mail atabilirsin.
          ...
          <Reveal delay={0.1}>
            <div className="rounded-3xl border p-6 space-y-3" style={{ background: "rgb(var(--surface))", borderColor: "rgb(var(--surface2))" }}>
              <h2 className="text-sm font-bold mb-1" style={{ color: "rgb(var(--t1))" }}>Sosyal Medya</h2>
